...
function generateRandomIPv6FromCIDR(cidr) {
    ...
    // If there are missing segments (due to "::"), fill them with random values
    for (let i = 0; i < 8; i++) {
    ...
    // Join the result in the correct order (should be left to right)
    return result.join(':');
}
...
// تغییر در اینجا: حذف نمایش خودکار رنج‌ها هنگام تغییر کشور
countrySelect.addEventListener('change', () => {
    const country = countrySelect.value;
    // فقط کشور انتخاب شده را ذخیره می‌کنیم و فیلدها را خالی می‌کنیم
...
document.getElementById("generate-dns").addEventListener("click", () => {
    ...
    if (country && dnsData[country]) {
        ...
        console.log("Random IPv4 Ranges:", randomIPv4Ranges);  // Log selected IPv4 ranges
        console.log("Random IPv6 Ranges:", randomIPv6Ranges);  // Log selected IPv6 ranges
